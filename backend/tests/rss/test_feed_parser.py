...
def test_parse_valid_feed(mock_feed_valid):
    ...
    assert articles[0]["url"] == "https://example.com/test-article", "❌ URL parsing failed."
    assert articles[0]["summary"] == "This is a test article summary.", "❌ Summary parsing failed."
    assert articles[0]["published_at"] == "Wed, 20 Feb 2025 12:00:00 GMT", "❌ Published date parsing failed."
...
def test_parse_feed_missing_fields(mock_feed_missing_fields):
    ...
    assert articles[0]["title"] == "Test Article", "❌ Title should be correctly parsed."
    assert articles[0]["url"] == "https://example.com/test-article", "❌ URL should be correctly parsed."
    assert "summary" not in articles[0] or articles[0]["summary"] is None, "❌ Should not have summary if missing in feed."
    assert "published_at" not in articles[0] or articles[0]["published_at"] is None, "❌ Should not have published date if missing."
